...
SwiftUI 页面通过统一状态协调层调用学习规则、Review、附件和导出能力。项目数据进入 SwiftData，附件保存在本地目录，AI 配置分别进入偏好设置与 Keychain；外部 AI 服务是可选边界。
...
Review 生成后立即保存并写入相关 Project Trail，同时显示 Facts、Patterns、Decisions、Next Steps 和来源。之后可以再次编辑；保存 Review 不等于接受建议，状态与 Next Step 分别通过显式按钮应用。
