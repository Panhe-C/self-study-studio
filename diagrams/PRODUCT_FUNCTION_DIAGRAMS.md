...
当前主导航由 Today、Projects、Library 三个 Tab 组成。Review 按条件从 Today 或 Project Detail 进入；AI Review Settings 只在需要复盘时随 Review 区域出现，不是根页面或常驻 Tab。

## 3. 功能模块关系

![当前功能模块关系](./product-functional-modules.svg)

SwiftUI 页面通过统一状态协调层调用学习规则、Review、附件和导出能力。Journal Store 正常使用 SwiftData，初始化失败时降级到 JSON Store；导出接收内存 Snapshot，不直接读取数据库。附件保存在本地目录，AI 配置分别进入偏好设置与 Keychain，外部 AI 服务是可选边界。
...
Review 生成后立即保存；当结果包含具体项目的 Next Step 或状态建议时，才会同步写入相关 Project Trail。页面显示 Facts、Patterns、Decisions、Next Steps 和来源，内容可以再次编辑；保存 Review 不等于接受建议，状态与 Next Step 分别通过显式按钮应用。
...
Library 当前只有一个 Export 操作：它创建包含版本化 `journal.json` 和按 Project/Session/Proof 整理附件的完整 Bundle，并通过提示框告知本地保存路径。失败不会修改或删除原始数据。
