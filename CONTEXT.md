...
_Gap_: Repository purge commits first and project-scoped attachment cleanup is retryable through a local private queue only after a canonical purge tombstone is present; legacy queues migrate project IDs when unambiguous and surface allowlisted ImportedAssets/attachment-root paths as explicitly confirmed orphan cleanup, while unsafe paths remain visible for repair. Corrupt queues can be quarantined into shareable timestamped artifacts before a new queue is accepted. Trash exports are unencrypted and require trusted-location confirmation; physical-device acceptance remains unverified.
